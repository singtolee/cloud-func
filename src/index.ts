...
export const baidufanyitoth = functions.https.onRequest((req,res)=>{
    const text = req.query.text;
    const url = BaiduTrans(text);
    request(url,function(err,resp,body){
        if (!err) {
            const aa = JSON.parse(body).trans_result[0].dst
            res.send(aa)
         
        } else {
            console.log('BAIDUFANYIERROR', err)
            res.send(text)
        }
...
export const mstoth = functions.https.onRequest((req,res)=>{
    const text = req.query.text;
    client.translate({ text: text, from: 'zh-cn', to: 'th' }, function (err, data) {
        if (err) {
            console.error("MSTRANSLATEERROR", err)
            res.send(text);
        } else {
            res.send(data);
        }
...
export const mstocn = functions.https.onRequest((req,res)=>{
    const text = req.query.text;
    client.translate({ text: text, to: 'zh-cn' }, function (err, data) {
        if (err) {
            console.error("MSTRANSLATEERROR", err)
            res.send(text);
        } else {
    ...
    if (data.error_code === 0) {
        ...
        for (const sku of sku_detail) {
            if (isOne(sku.sku_name)) {
                ...
                if (sku.sku_name.includes('>')) { //1688 product
                    let cc = sku.sku_name.split(/\>/);
                    if (isNumber(cc[1])) {
                        sku.sku_thName = cc[1]
                    } else {
                        sku.sku_thName = await callapi(cc[1])
                    }
...
function BaiduTrans(query:string){
    const FANYIURL = 'https://fanyi-api.baidu.com/api/trans/vip/translate';
    const appid = '20180814000194093';
    const key = 'c4iefMkFKo4wofRHMMoR';
    ...
    const url = FANYIURL + '?q=' + query + '&from=' + from + '&to=' + to + '&appid=' + appid + '&salt=' + salt + '&sign=' + sign;
    return url
}
